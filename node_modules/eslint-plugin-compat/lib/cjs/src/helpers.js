...
function lintLiteral(context, handleFailingRule, rules, sourceCode, node) {
    const isRegexpLiteral = checkRegexpLiteral(node);
    const failingRule = rules.find((rule) => rule.syntaxes?.some((syntax) => (isRegexpLiteral ? node.raw.includes(syntax) : false) // non-regexp literals are not supported yet
    ));
    if (failingRule)
        handleFailingRule(failingRule, node);
}
...
function lintMemberExpression(context, handleFailingRule, rules, sourceCode, node) {
    ...
    if (!node.object.name ||
        ...
        const failingRule = rules.find((rule) => rule.object.toLowerCase() === objectName.toLowerCase() &&
